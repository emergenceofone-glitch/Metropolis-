...
const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none p-4 font-sans z-20 flex flex-col items-center">
        ...
        <AnimatePresence>
          {showHoverInfo && hoveredConfig && (
            <motion.div
              ...
              exit={{ x: -20, opacity: 0 }}
              className="bg-slate-950/40 backdrop-blur-md rounded-2xl border border-white/10 p-3 min-w-[180px] shadow-2xl"
            >
              ...
              {!isMaxLevel && selectedTool === hoveredTile?.buildingType && (
                 <div className="mt-3 pt-2 border-t border-white/5">
                    <div className="flex items-center justify-between gap-2">
                       <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Upgrade</span>
                       <span className={`text-[10px] font-mono ${canAffordUpgrade ? 'text-white' : 'text-rose-400'}`}>${upgradeCost}</span>
                    </div>
                    <div className="text-[9px] text-cyan-400/70 mt-1 italic leading-tight text-center">Click again to enhance</div>
                 </div>
              )}
            </motion.div>
          )}
      ...
      {/* AI Advisor - Top Right Float */}
      <div className="absolute top-4 right-4 flex flex-col items-end gap-2 w-64 md:w-80 pointer-events-auto">
        <AnimatePresence mode="wait">
          {aiEnabled && (
            <motion.div 
              layout
          ...
          {latestNews && (
            <motion.div
              ...
              className={`flex items-start gap-2 p-3 rounded-xl border backdrop-blur-md shadow-xl ${
                latestNews.type === 'positive' ? 'bg-emerald-950/40 border-emerald-500/30 text-emerald-200' :
                latestNews.type === 'negative' ? 'bg-rose-950/40 border-rose-500/30 text-rose-200' :
                'bg-slate-950/40 border-white/10 text-slate-200'
              }`}
            >
      ...
      <div className="mt-auto pointer-events-auto flex flex-col items-center gap-3 pb-4">
        <motion.div 
          initial={{ y: 100 }}
          animate={{ y: 0 }}
          className="flex items-center gap-1.5 p-1.5 bg-slate-950/40 backdrop-blur-xl rounded-2xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
        >
          {tools.map(({ type, icon: Icon, color }) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon className={`w-5 h-5 md:w-6 md:h-6 ${isSelected ? 'text-white' : color} transition-colors`} />
