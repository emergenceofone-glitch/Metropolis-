    ...
    >
      ...
      <div className="p-4 bg-slate-900/50 border-b border-white/10 flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 scrollbar-none text-xs font-mono">
          ...
          {(['milestone', 'goal', 'disaster', 'directive', 'historical', 'weather'] as ChronicleCategory[]).map(cat => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl border flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:outline-none ${
                  isActive
                    ? `${cfg.bg} ${cfg.border} ${cfg.text} font-bold shadow-md`
                    : 'bg-slate-800/80 border-white/10 text-slate-200 hover:text-white'
                }`}
              >
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-48">
            ...
            <input
              ...
              aria-label="Search City Chronicle"
              value={searchQuery}
              onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
              className="w-full bg-slate-950 border border-white/20 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-amber-400 focus-visible:ring-2 focus-visible:ring-amber-400 font-mono"
            />
            {searchQuery && (
              <button 
                onClick={() => setSearchQuery('')}
                className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-300 hover:text-white text-xs p-1"
